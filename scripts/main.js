...
function appearance() {
  return { users: game.users, showLabels: game.settings.get(ID, "labels"),
...
function refreshAppearance() {
  ...
  for (const ref of renderedCards) {
    ...
    if (!html) { renderedCards.delete(ref); continue; }
...
export function compatibilityProblem(environment, api = globalThis.foundry) {
  ...
  if (typeof Message?.create !== "function" || typeof Message?.applyMode !== "function" ||
      typeof Message?.prototype?.updateSource !== "function" || typeof Message?.prototype?.renderHTML !== "function" ||
      typeof api?.utils?.randomID !== "function" || typeof environment.users?.get !== "function" ||
      typeof globalThis.DOMParser !== "function" || typeof globalThis.WeakRef !== "function" ||
      !["author", "blind", "whisper", "rolls", "content", "speaker", "flags"].every(key => fields?.[key])) {
...
Hooks.once("init", () => {
  ...
  game.settings.register(ID, "labels", {
    ...common, scope: "client", onChange: refreshAppearance,
    name: "Show roll visibility labels", type: Boolean, default: true,
    hint: "Show mode labels on your chat cards, including your blind-roll placeholders."
  });
  game.settings.register(ID, "borders", {
    ...common, scope: "client", onChange: refreshAppearance,
    name: "Add coloured roll borders", type: Boolean, default: true,
    hint: "Show a coloured border on your roll cards. This works independently of labels."
  });
  game.settings.register(ID, "audience", {
    ...common, scope: "world", name: "Blind-roll confirmation audience", type: String, default: "self",
  ...
  game.settings.register(ID, "detail", {
    ...common, scope: "world", name: "Blind-roll confirmation detail", type: String, default: "check",
...
Hooks.once("ready", () => {
  ...
  if (problem) { ui.notifications.warn(`GGA Roll Clarity: ${problem}`); return; }
  if (Number(game.release.generation) > 14) {
    ui.notifications.warn("GGA Roll Clarity has not been verified with this Foundry version. Check blind-roll visibility before play.");
  }
  active = true;
  const settings = () => ({
